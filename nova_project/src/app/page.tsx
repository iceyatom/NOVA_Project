...
type Health = {
  ...
  timestamp?: string
  components?: {
    db?: { ok: boolean; dbName?: string | null; serverTime?: string | null; name?: string; code?: string; message?: string }
    s3?: { ok: boolean; skipped?: boolean; name?: string; code?: string; message?: string }
  }
}

function StatusBadge({ state }: { state: 'loading' | 'ok' | 'error' }) {
  const text = state === 'loading' ? 'Checking AWS…' : state === 'ok' ? 'AWS connection successful' : 'AWS connection failed'
    ...
    return (
      <div className="mt-3 text-sm text-gray-700">
        ...
        <div>
          ...
          {db?.ok ? (
            <>
              connected{db?.dbName ? ` to ${db.dbName}` : ''} {db?.serverTime ? `• server time ${new Date(db.serverTime).toLocaleString()}` : ''}
            </>
          ) : (
            <>
              error{db?.code ? ` (${db.code})` : ''} {db?.message ? `— ${db.message}` : ''}
            </>
          )}
        </div>
        ...
        {s3 && (
          <div className="mt-1">
            <strong>S3:</strong>{' '}
            {s3.skipped ? 'skipped (not configured)' : s3.ok ? 'ok' : `error${s3.code ? ` (${s3.code})` : ''}${s3.message ? ` — ${s3.message}` : ''}`}
          </div>
        )}
